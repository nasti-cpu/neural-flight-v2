...
import { getHeight, DEFAULT_HEIGHTMAP, type HeightmapConfig } from "./terrain/heightmap";
...
export function createChunkRings(
	...
	for (let i = 0; i < RINGS.PER_CHUNK; i++) {
		...
		rings.push({ mesh, collected: false, center: new THREE.Vector3(wx, y, wz) });
	}
...
export function updateRings(rings: RingState[], playerPos: THREE.Vector3): number {
	let collected = 0;
