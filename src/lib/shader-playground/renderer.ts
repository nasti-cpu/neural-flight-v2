...
import { parseShaderErrors } from "./compiler";
...
export function createPlaygroundRenderer(
	...
	let currentGeometryType: GeometryType = "plane";
	...
	function updateShader(
		...
		// Test compile fragment
		const fragErrors = testCompileFragment(gl, fragment);
		errors.push(...fragErrors);

		// Test compile vertex (if provided)
		if (vertex) {
			const vertErrors = testCompileVertex(gl, vertex);
			errors.push(...vertErrors);
		}
...
function testCompileFragment(
	...
	gl.deleteShader(shader);
	return parseShaderErrors(log);
}

function testCompileVertex(
	gl: WebGLRenderingContext | WebGL2RenderingContext,
	source: string,
): ShaderError[] {
