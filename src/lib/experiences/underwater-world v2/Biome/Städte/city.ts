...
export function createCity(variant: CityVariant): CityResult {
  ...
  // ── Geodesic dome ──
  // Standard material instead of Physical (no clearcoat layer) — big GPU
  // saving for a large transparent doubleside dome; visually nearly identical.
  ...
  for (let i = 0; i < cfg.buildingCount; i++) {
    ...
    buildings.push({
      ...
      color:
        cfg.buildingColors[
          Math.floor(Math.random() * cfg.buildingColors.length)
        ],
    });
  }
