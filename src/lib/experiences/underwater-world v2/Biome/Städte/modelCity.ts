...
async function _loadGLB(): Promise<void> {
	try {
		...
		const bmin = box.min;
		_cachedOffset.set(-center.x * _cachedScale, -bmin.y * _cachedScale, -center.z * _cachedScale);
...
function _createModelCityScene(): ModelCityResult {
	...
	const domeGeo = new THREE.SphereGeometry(_domeRadius, 32, 24, 0, Math.PI * 2, 0, Math.PI / 2);
...
export async function createModelCity(variant: ModelCityVariant): Promise<ModelCityResult> {
	await _loadPromise;
	return _createModelCityScene();
}
...
export function disposeModelCity(result: ModelCityResult, scene: THREE.Scene): void {
