...
// Procedural environment replacing texture(iChannel0, reflected*reflected*reflected)
vec3 fakeEnv(vec3 rd) {
    float t = 0.5 + 0.5 * rd.y;
    vec3 sky = mix(vec3(0.1, 0.1, 0.2), vec3(0.4, 0.6, 1.0), t);
    float sun = pow(max(dot(rd, normalize(vec3(1.0, 1.0, 0.5))), 0.0), 32.0);
    return sky + vec3(1.0, 0.8, 0.5) * sun * 0.3;
}
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    vec3 env = fakeEnv(reflected * reflected * reflected);
