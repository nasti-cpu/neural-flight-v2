<script lang="ts">
	...
	onMount(() => {
		...
		async function loadStones() {
			const stoneScene = await loadGLB("/models/steine/mossy_stones_pack_tliiadmva_raw.glb");
			...
			for (let pi = 0; pi < parts.length; pi++) {
				const { geo, mat } = parts[pi];
				const count = pi === 1 ? 3 : 5;
...
<div class="container">
	...
	<div class="ui-overlay">
		<h1>🪨 Wiese mit Steinen</h1>
		{#if loading}
			<p class="loading">Lade Stein-Modell (384 MB) …</p>
		{/if}
