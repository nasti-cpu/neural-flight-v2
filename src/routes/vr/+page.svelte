<script lang="ts">
    ...
    let renderer: any; // THREE.WebGLRenderer | THREE.WebGPURenderer
    ...
    onMount(() => {
        ...
        // Async init wrapper (Svelte 5 onMount can't be async)
        (async () => {
            let useWebGPU = false;
            try {
                const { getExperience } =
                    await import("$lib/experiences/catalog");
                const manifest = getExperience(experienceId);
                useWebGPU = manifest.rendererMode === "webgpu";
            } catch {
                // Fallback: WebGL
            }
            ...
            loadExperience(experienceId, {
                ...
                renderer.setAnimationLoop(() => {
                    ...
                    const result = exp.manifest.tick(exp.state, {
                        ...
                        camera: renderCamera,
                        playerPosition:
                            renderCamera.parent?.position ?? _fallbackPos,
                        playerRotation:
                            renderCamera.parent?.rotation ?? _fallbackRot,
                    });
